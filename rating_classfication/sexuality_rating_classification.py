...
def remove_timeline_from_text(input_file):
    ...
    print(f"Reading file: {input_file}")  # 파일 읽기 경로 확인
...
def classify_sexuality_rating(input_img_path, input_text_path, output_file): 
    ...
    with open(input_img_path, "r", encoding="utf-8") as file:
        ...
        if isinstance(image_data, list):
            image_data = image_data[-1]  # 마지막 객체 선택
        else:
            image_data = image_data  # 그대로 사용
    ...
    rating_criteria = """
    <선정성>
    [All Ages]: No or very minimal sexual content or nudity, with almost no suggestive elements.
    [12+]: Mild and brief sexual content or nudity, with suggestive elements being subtle and limited.
    [15+]: Sexual content or nudity is present but not explicit, prolonged, or highly detailed.
    [(18+)Adults Only]: Sexual content or nudity is highly explicit, direct, and graphically detailed.
    """

    # 프롬프트
    prompt = f'''
    아래 제시한 대사와 이미지 분석 데이터를 보고 오직 선정성 분류 기준에 따라 영상물의 선정성 등급을 판정하고, 등급 판정의 이유를 출력하시오.
    문맥을 고려하여 판정하고, 15세보다는 12세로 판정하도록 노력하시오.
...
if __name__ == "__main__":
    ...
    output_file = 'C:/Users/chloeseo/ms_project/test_v6/result/수리남/수리남/테스트.json' # 결과 파일 경로
